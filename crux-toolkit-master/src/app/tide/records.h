...
public:
        ...
        bool Read(google::protobuf::Message* message) {
                ...
                //The value returned by PushLimit() is opaque to the caller,
                //and must be passed unchanged to the corresponding call to PopLimit().
...
public:
        HeadedRecordReader(const string& filename, pb::Header* header = NULL,
                           int buf_size = -1)
                : reader_(filename, buf_size), header_(header), // line 169 for RecordReader
        ...
        const pb::Header* GetHeader() const {
                return header_;
        }
